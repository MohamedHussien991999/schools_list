...
class CustomBodyFilter extends StatelessWidget {
  const CustomBodyFilter({super.key});
                ...
                child: Column(
                  children: [
                    CustomDropDown(
                      ...
                      items: const [
                        DropdownMenuItem(
                            value: 'type', child: Text('Type School')),
                        DropdownMenuItem(value: 'level', child: Text('Level')),
                        DropdownMenuItem(
                            value: 'language', child: Text('Language')),
                      ],
                      selectedValue: SchoolsCubit.get(context).filterChoice,
                      onChanged: (value1) {
                        selectedValue = value1!;
                        selectedValue2 = '';
                        SchoolsCubit.get(context).filterChoice = value1;
                        SchoolsCubit.get(context).changeFilterType(value1);
                      },
                    ...
                    CustomDropDown(
                      ...
                      items: SchoolsCubit.get(context).filterChoiceList,
                      selectedValue:
                          SchoolsCubit.get(context).filterChoiceList?[0].value,
                      onChanged: (value2) {
                    ...
                    Expanded(
                      child: ListView.builder(
                        itemBuilder: (context, index) => SchoolItem(
                          schoolModel:
                              SchoolsCubit.get(context).filterSchools[index],
                        ),
                        itemCount:
